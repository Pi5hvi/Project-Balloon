...
{
  if (count>0){
    vert.add(new PVector(mouseX-cx,mouseY-cy));
    point(mouseX,mouseY);
...
//}
    ...
    //int g=i-2;
    //int h=i-1;  // For the index of previous two points
    //float m1 = (x[g] - x[h])/(y[h]-y[g]);  // slope of perpendicular bisectors to lines AB
    //float m2 = (x[h] - x[i])/(y[i]-y[h]);  // slope of perpendicular bisectors to lines BC
    //float aby = (y[g]+y[h])/2;   //midpoint Y of AB
    //float abx = (x[g]+x[h])/2;   //midpoint X of AB
    //float cby = (y[i]+y[h])/2;   //midpoint Y of AB
    //float cbx = (x[i]+x[h])/2;   //midpoint X of AB
    //cx[j]=(cby-aby+m1*abx-m2*cbx)/(m1-m2);
    //cy[j]=m1*(cx[j]-abx)+aby;
    //d[j]=2*abs(sqrt(sq(y[i]-cy[j])+sq(x[i]-cx[j])));
      if (abs((vert.get(h).y - vert.get(g).y))>0.3&&abs((vert.get(i).y - abs(vert.get(h).y)))>0.3){
        if(m1!=m2){
          cent.set(j,new PVector(((cby-aby+m1*abx-m2*cbx)/(m1-m2)-cx),(m1*((cby-aby+m1*abx-m2*cbx)/(m1-m2)-abx)+aby)-cy));
        }
        else{
          cent.set(j,new PVector((-cx+(cby-aby+m1*abx-m2*cbx)/(m1-m2)),(m1*((cby-aby+m1*abx-m2*cbx)/(m1-m2)-abx)+aby)-cy));
        }
      }
      if (abs((vert.get(h).y - vert.get(g).y))<0.3){
        cent.set(j,new PVector((((((vert.get(g).y+vert.get(h).y)/2-cbx)/m2)+cbx)-cx),((vert.get(g).y+vert.get(h).y)/2)-cy));
      }
      if (abs((vert.get(i).y - vert.get(h).y))<0.3){
        cent.set(j,new PVector(((((vert.get(i).y+vert.get(h).y)/2-abx)/m1)+cbx)-cx,((vert.get(i).y+vert.get(h).y)/2)-cy));
      }
      ...
      if (start==finish||mid==finish||start==mid){
        ...
        arc(cent.get(j).x,cent.get(j).y,dia.get(j),dia.get(j),start,finish);
                for (int l=0;l<10;l++){
          points.add(new PVector(cos(start+(finish-start)*l/10)*dia.get(j)/2+cent.get(j).x,sin(start+(finish-start)*l/10)*dia.get(j)/2+cent.get(j).y));          
        }
        //for(int l=0;l<points.size();l++){
        //  stroke(255,0,0);
        //  point(points.get(l).x,points.get(l).y);
        //}
        println("DONE"+points.size(),"P1:"+vert.get(g),"P2:"+vert.get(0),"cent:"+cent.get(j),"rad:"+dia.get(j));
        k=10;
      }
      if (k==0){
      ...
      //if (i==19){
   // in//t g=i-1;
    //in//t h=i;  // For the index of previous two points
    //i=//0;
    //fl//oat m1 = (x[g] - x[h])/(y[h]-y[g]);  // slope of perpendicular bisectors to lines AB
    //fl//oat m2 = (x[h] - x[i])/(y[i]-y[h]);  // slope of perpendicular bisectors to lines BC
    //fl//oat aby = (y[g]+y[h])/2;   //midpoint Y of AB
    //fl//oat abx = (x[g]+x[h])/2;   //midpoint X of AB
    //fl//oat cby = (y[i]+y[h])/2;   //midpoint Y of AB
    //fl//oat cbx = (x[i]+x[h])/2;   //midpoint X of AB
    //cx//[j]=(cby-aby+m1*abx-m2*cbx)/(m1-m2);
    //cy//[j]=m1*(cx[j]-abx)+aby;
    //d[//j]=2*abs(sqrt(sq(y[i]-cy[j])+sq(x[i]-cx[j])));
    //fl//oat start=atan2(y[g]-cy[j],x[g]-cx[j]);
    //fl//oat startd=start*180/PI;
    //fl//oat mid=atan2(y[h]-cy[j],x[h]-cx[j]);
    //fl//oat midd=mid*180/PI;
    //fl//oat finish=atan2(y[i]-cy[j],x[i]-cx[j]);
    //fl//oat finishd=finish*180/PI;
    //////float start= atan((y[g]-cy[j])/(x[g]-cx[j]));
    ////float finish= atan((y[i]-cy[j])/(x[i]-cx[j]));
    //st//roke(255,255,255);
    //no//Fill();
    //if// (startd<midd&&midd<finishd){
    //ar//c(cx[j],cy[j],d[j],d[j],start,finish);
    //}
//    //if (finishd<midd&&midd<startd){
    //ar//c(cx[j],cy[j],d[j],d[j],finish,start);
    //}
//    ////if (startd<midd&&finish<start){
    ...
    ////arc(cx[j],cy[j],d[j],d[j],start,finish);
    ////}
    //pr//intln("centre:"+cx[j],""+cy[j],"Dia:"+d[j],"Start"+startd,"Mid:"+midd,"Finish:"+finishd);
  //}
      float startd=start*180/PI;
      float midd=mid*180/PI;
      float finishd=finish*180/PI;
      ...
      if (finishd<midd&&midd<startd){
        arc(cent.get(j).x,cent.get(j).y,dia.get(j),dia.get(j),finish,start);
                for (int l=0;l<10;l++){
          points.add(new PVector(cos(finish+(start-finish)*l/10)*dia.get(j)/2+cent.get(j).x,sin(finish+(start-finish)*l/10)*dia.get(j)/2+cent.get(j).y));          
        }
      ...
      //arc(cx[j],cy[j],d[j],d[j],start,finish);
      //}
      println("centre:"+cent.get(j).x,""+cent.get(j).y,"Dia:"+dia.get(j),"Start"+startd,"Mid:"+midd,"Finish:"+finishd,"S1:"+strt1,"M1:"+mid1,"F1:"+fin1,"pox:"+points.get((points.size()-1)));
      j=j+1;
    }
    }
  i=i+1;
